...
在开发阶段，使用微信开发者工具打开对应的 H5 页面，使用 `getLocation` 获取定位信息，定位结果是偏移比较大的。哪怕是直接在微信里访问 H5 页面，定位结果也是偏移比较大的。据说是因为微信在 H5 页面中获取定位信息时，会对定位结果进行偏移处理，以保护用户隐私。
但嵌套在小程序中的 H5 页面，假如是在微信开发者工具中在小程序中打开 H5 页面，是无法弹出定位授权的。这时候就需要在真机小程序中打开 H5 页面进行定位。但在真机小程序中打开 H5 页面进行定位时，定位结果却是比较准确的。

迷惑……持续的迷惑……，无语……持续的无语……

但至始至终也只能是经纬度信息，无法获取到详细的地址信息。比如：省市区、街道等。这时候就再需要其他的服务来获取详细的地址信息。比如腾讯自己的[地图服务](https://lbs.qq.com/service/webService/webServiceGuide/address/Gcoder)。